#ifndef OMNIX_CUSTOM_CONTROLLER_RAW_PROTOCOL_H
#define OMNIX_CUSTOM_CONTROLLER_RAW_PROTOCOL_H
...
#define OMNIX_JOINT_RAW_COUNT                  7u
#define OMNIX_CUSTOM_CONTROLLER_PROTOCOL_VERSION 2u
#define OMNIX_CUSTOM_CONTROLLER_RX_TIMEOUT_MS  3000u
...
#define OMNIX_CTRL_J5_RAW_MAX  1086u
...
static const omnix_controller_joint_raw_calib_t OMNIX_CONTROLLER_JOINT_RAW_CALIB[OMNIX_JOINT_RAW_COUNT] = {
...
{
    if (joint_id < 1u || joint_id > OMNIX_JOINT_RAW_COUNT) {
        return 0;
    }
...
static inline int16_t Omnix_DegToCdeg(float angle_deg)
{
    ...
    int32_t angle_cdeg = (int32_t)((scaled >= 0.0f) ? (scaled + 0.5f) : (scaled - 0.5f));
