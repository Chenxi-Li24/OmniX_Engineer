...
struct LK8016EState {
  ...
  uint32_t last_encoder_rx_ms = 0;
  uint32_t last_multi_turn_rx_ms = 0;
  ...
  bool     multi_turn_valid = false;
  int64_t  multi_turn_angle_cdeg = 0;
  fp32     multi_turn_angle_deg = 0.0f;
  uint8_t  last_cmd = 0;        // last feedback cmd (A1..A8)
  uint32_t rx_count = 0;
  uint32_t encoder_rx_count = 0;
  uint32_t multi_turn_rx_count = 0;
};

inline constexpr fp32 LK8016E_ENCODER_RAW_TO_DEG = 360.0f / 65536.0f;

inline constexpr fp32 LK8016E_EncoderRawToDeg(uint16_t encoder_raw)
{
  return static_cast<fp32>(encoder_raw) * LK8016E_ENCODER_RAW_TO_DEG;
}
...
public:
  ...
  void request_stop();
  void request_read_single_turn_encoder();
  void request_read_multi_turn_angle();
